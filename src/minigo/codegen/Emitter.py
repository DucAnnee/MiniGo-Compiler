...
class Emitter:
    ...
    def getJVMType(self, inType):
        ...
        elif typeIn in (cgen.ClassType, cgen.Id):
    ...
    *    @param typ the type of the constant
    ...
    def emitPUSHCONST(self, in_, typ, frame):
        ...
        if type(typ) is BoolType:
            return (
                self.emitPUSHICONST(1, frame)
                if (in_ == "true")
                else self.emitPUSHICONST(0, frame)
            )
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitVAR(self, in_, varName, inType, fromLabel, toLabel, frame):
        ...
        type = self.getJVMType(inType)
        if isinstance(inType, ArrayType):
            dimen_len = len(inType.dimens)
            type = "[" * dimen_len + self.getJVMType(inType.eleType)
        return self.jvm.emitVAR(in_, varName, type, fromLabel, toLabel)
    ...
    def emitATTRIBUTE(self, lexeme, in_, isStatic, isFinal, value):
        ...
        if isStatic:
            ...
            return self.jvm.emitINSTANCEFIELD(
                "public " + lexeme, self.getJVMType(in_), isFinal, value
            )
    ...
    *   .super java/lang/Object<p>
    ...
    def emitPROLOG(self, name, parent, isInterface=False):
        # name: String
        # parent: String
        ...
        if isInterface:
            result.append(self.jvm.emitCLASS(f"public interface {name}"))
        else:
            result.append(self.jvm.emitCLASS(f"public {name}"))
        result.append(
    ...
    *   @param in the code to be printed out
    ...
    def emitNEWARRAY(self, in_, frame):
        if isinstance(in_, IntType):
            ...
            return self.jvm.emitNEWARRAY("Ljava/lang/String")
        elif isinstance(in_, Id):
            return self.jvm.emitANEWARRAY(in_.name)
        else:
            raise IllegalOperandException(in_)

    def emitANEWARRAY(self, in_, frame):
        type = self.getJVMType(in_)
        return self.jvm.emitANEWARRAY("[" + type)

    def emitMULTIANEWARRAY(self, in_, frame):
        dimens = len(in_.dimens)
        type = self.getJVMType(in_)
        return self.jvm.emitMULTIANEWARRAY(type, str(dimens))
