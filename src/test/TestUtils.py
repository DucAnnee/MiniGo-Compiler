...
if (
    os.path.isdir("../target/main/minigo/parser")
    and not "../target/main/minigo/parser/" in sys.path
):
...
class TestCodeGen:
    ...
    def check(soldir, asttree, num):
        ...
        try:
            ...
            subprocess.call(
                "java  -jar " + JASMIN_JAR + " " + path + "/*.j",
                shell=True,
                stderr=subprocess.STDOUT,
            )

            subprocess.run(
                "java -cp ./lib;. MiniGoClass", shell=True, stdout=f, timeout=10
            )
        except StaticError as e:
            f.write(str(e))
